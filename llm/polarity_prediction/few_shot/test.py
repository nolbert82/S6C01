...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Few-shot polarity prediction with LM Studio (ministral3)."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output_csv",
        type=Path,
        default=Path("llm/polarity_prediction/few_shot/predictions.csv"),
        help="Where to write row-level predictions.",
    )
    parser.add_argument(
        "--results_path",
        type=Path,
        default=Path("llm/polarity_prediction/few_shot/results.txt"),
...
def build_prompt(review_text: str) -> str:
    return (
        "Classify the sentiment of Yelp reviews into one label: negative, neutral, or positive.\n"
        "Use the examples as guidance. Output exactly one label for the final review.\n\n"
        "Example 1:\n"
        "Review: [PUT YOUR EXAMPLE REVIEW 1 HERE]\n"
        "Label: [PUT LABEL: negative|neutral|positive]\n\n"
        "Example 2:\n"
...
def main() -> None:
    ...
    result_text = (
        "LLM Few-Shot Polarity Prediction (ministral3 via LM Studio)\n"
        f"Input: {args.input}\n"
