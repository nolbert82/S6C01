...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a frozen DistilBERT + GRU model for score prediction."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Hidden size of the GRU head.",
    )
    parser.add_argument(
        "--gru_num_layers",
        type=int,
...
def main() -> None:
    ...
    if labels.min() < 1 or labels.max() > 5:
        raise ValueError("score_label must be in [1, 5] for this model.")
    y_train = labels.to_numpy() - 1
    ...
    optimizer = torch.optim.Adam(gru_head.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    n_rows = len(documents)
    steps_per_epoch = math.ceil(n_rows / args.batch_size)
    print(
        f"Training frozen BERT + GRU for {args.epochs} epoch(s), batch_size={args.batch_size}, "
    ...
    model_path = args.output_dir / "bert_gru_score_model.pt"
    bundle = {
        "gru_state_dict": gru_head.state_dict(),
