...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test fine-tuned DistilBERT score model on the testing dataset."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--model_dir",
        type=Path,
        default=Path("models/bert_finetune/score_prediction/saved_model"),
        help="Directory containing the trained fine-tuned BERT model.",
    )
    parser.add_argument(
        "--results_path",
        type=Path,
        default=Path("models/bert_finetune/score_prediction/results.txt"),
...
def main() -> None:
    ...
    model_path = args.model_dir / "bert_finetune_score_model.pt"
    ...
    dataset = TextLabelDataset(texts, (y_true - 1).tolist())
