...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune DistilBERT for score prediction (1 to 5)."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def main() -> None:
    ...
    if labels.min() < 1 or labels.max() > 5:
        raise ValueError("score_label must be in [1, 5].")
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        args.bert_model_name, num_labels=5
    )
    ...
    model_path = args.output_dir / "bert_finetune_score_model.pt"
    bundle = {
