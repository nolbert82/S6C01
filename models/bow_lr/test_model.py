...
def load_and_join_data():
    ...
    try:
        ...
        # For testing purposes, we might want to use a subset if the file is too large
        # but let's try to load it all and take a sample for evaluation
        df_reviews = pd.read_csv(review_path).sample(n=10000, random_state=42) # Evaluation on a sample for speed
...
def test():
    ...
    if not os.path.exists(model_path) or not os.path.exists(vectorizer_path):
        print("Error: Saved model or vectorizer not found. Please run train_model.py first.")
        sys.exit(1)
    ...
    with open(config_path, "r") as f:
        line = f.read()
        input_dim = int(line.split("=")[1])
        
    # 2. Load model
    print("Loading model...")
    model = LinearModel(input_dim, 5).to(device)
    ...
    X_test = vectorizer.transform(text_data).toarray()
