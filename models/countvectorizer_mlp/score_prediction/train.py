...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a PyTorch MLP model to predict score_label using a shared CountVectorizer."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=Path("models/countvectorizer_mlp/score_prediction/saved_model"),
...
def main() -> None:
    ...
    warnings.filterwarnings(
        "ignore",
        message="Pyarrow will become a required dependency of pandas in the next major release of pandas",
    ...
    if labels.min() < 1 or labels.max() > 5:
        raise ValueError("score_label must be in [1, 5] for this MLP model.")

    y_train_np = labels.to_numpy() - 1
    ...
    model_path = args.output_dir / "mlp_score_model.pt"
