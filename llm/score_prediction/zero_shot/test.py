...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Zero-shot score prediction with LM Studio (ministral3)."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output_csv",
        type=Path,
        default=Path("llm/score_prediction/zero_shot/predictions.csv"),
        help="Where to write row-level predictions.",
    )
    parser.add_argument(
        "--results_path",
        type=Path,
        default=Path("llm/score_prediction/zero_shot/results.txt"),
...
def build_prompt(review_text: str) -> str:
    return (
        "Read the Yelp review and estimate:\n"
        "1) overall_score: an integer from 1 to 5\n"
        "2) aspects: scores for mentioned aspects only, each as integer 1 to 5\n\n"
        ...
        "}\n"
        "Rules:\n"
        "- Include only aspects clearly mentioned.\n"
...
def main() -> None:
    ...
    result_text = (
        "LLM Zero-Shot Score Prediction + Aspect Scoring (ministral3 via LM Studio)\n"
        f"Input: {args.input}\n"
