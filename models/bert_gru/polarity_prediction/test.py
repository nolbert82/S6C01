...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test frozen DistilBERT + GRU polarity model on the testing dataset."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--results_path",
        type=Path,
        default=Path("models/bert_gru/polarity_prediction/results.txt"),
...
def main() -> None:
    ...
    model_path = args.model_dir / "bert_gru_polarity_model.pt"
    ...
    with torch.no_grad():
        for batch_idx in range(num_batches):
            ...
            pred_labels = np.array([label_values[i] for i in pred_idx], dtype=np.int64)
            preds_all.extend(pred_labels.tolist())
